...
- 유효 풀 크기 2·3·12·13명의 알고리즘 선택 경계
...
- 현재 10개 합성 fixture에서 계산한 p95 손실률 1% 이하
- 최대 손실률 3% 이하
- 운영 전체 배치 목표 90분 이내
...
현재 품질 비교는 3~12명에서 인원별 합성 fixture 하나씩 총 10개를 사용하므로 p95 인덱스가 사실상
최대값과 같다. 90분 목표는 assertion이 아니며, 24개 풀 순수 알고리즘 시간만 별도로 측정한다. DB 조회·
저장, 잠금과 네트워크 시간은 벤치마크에 포함되지 않는다.
