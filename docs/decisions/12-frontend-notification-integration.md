...
백엔드 구현 완료(#198 → #200/PR #201, `develop` merge됨), 실제 Firebase 프로젝트/서비스 계정 키로
로컬·운영(EC2) 양쪽 다 검증까지 마쳤다. 설계 배경은 [13-fcm-push.md](./13-fcm-push.md) 참고.

**FE가 필요한 Firebase 웹 설정값도 이미 발급 완료, 팀 채팅으로 전달했다** — `firebaseConfig`
(apiKey/authDomain/projectId/storageBucket/messagingSenderId/appId) + 웹 푸시 인증서(VAPID 키). 못 받았으면
백엔드 쪽에 다시 요청하면 된다(코드/문서로는 안 남겨둠 — 클라이언트 노출값이라 시크릿은 아니지만 굳이
git에 박아둘 값은 아님).
...
로컬·운영 양쪽 다 인증·API·발송 경로(FCM 서버까지 요청 도달, 인증 통과)는 확인했지만, **실제 기기에
알림이 뜨는지는 FE가 진짜 토큰으로 등록해봐야 확인 가능**하다 — 구조적으로 지금까지는 불가능했던 부분.
처음 연동할 때 뭔가 안 뜨면 알려달라, 같이 디버깅한다.
...
| 3 | FCM 토큰 저장/발송 배관 | ✅ 백엔드 완료, 로컬+운영 배포 검증(#200), FE 설정값 전달 완료, 프론트 미착수 |
