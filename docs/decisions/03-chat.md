...
- **실시간 갱신도 포함**: "읽음 처리는 실시간 브로드캐스트가 필요 없다"는 기존 결정(위 구현
  ...
  **(2026-08-06 PR #86 코드래빗 리뷰 반영)** 처음엔 안읽음 이후 메시지를 전부 조회하는 새
  ...
  자연스럽게 50건으로 제한되고, 발신자 N+1 문제도 같이 해결됐다. (2026-08-10 이슈 #115) 이후
  `getMessages` 쪽에는 이 메서드에 cursor 파라미터를 붙여 과거 메시지 페이지네이션을
  추가했지만, `markAsRead`는 여전히 화면에 보이는 범위(cursor 없이 최신 50건) 안에서만
  안읽음 갱신을 계산하도록 그대로 뒀다 — 위 무제한 방지 취지가 바뀌지 않았기 때문.
