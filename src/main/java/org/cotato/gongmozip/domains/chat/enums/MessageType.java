...
public enum MessageType {
    ...
    LEADER_VOTE_CARD,
    LEADER_VOTE_REMINDER_CARD, // 팀장 투표 마감 30분 전 리마인더 (메타데이터 없음, 버튼 라벨은 고정이고 클릭 시 이동할 화면만 프론트가 GET
    // .../leader-votes/status의 myVoted로 결정)
    LEADER_RESULT_CARD, // 팀장 확정 결과 카드 (아바타/이름 노출, 프로필 이동용)
    CONTEST_RECOMMEND_CARD,
    CONTEST_VOTE_CARD,
    CONTEST_VOTE_REMINDER_CARD, // 공모전 투표 마감 2시간 전 리마인더 (메타데이터 없음, 버튼 라벨은 고정이고 클릭 시 이동할 화면만 프론트가 GET .../votes의
    // myVoted로 결정)
